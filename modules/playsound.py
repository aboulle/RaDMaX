...
def _playsoundWin(sound, block = True):
    ...
    alias = b'playsound_' + str(random()).encode()
    sound = sound.encode()
    name = b'open "' + sound + b'" alias'
    winCommand(name, alias)
    winCommand(b'set', alias, b'time format milliseconds')
    durationInMS = winCommand(b'status', alias, b'length')
    winCommand(b'play', alias, b'from 0 to', durationInMS)
