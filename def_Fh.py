...
def f_f0(th,wl,el):
	...
	sum = coeff[5]
	while n <= 4:
		sum = sum + coeff[n] * exp ( -coeff[n+6] * ( sin(th) / wl )**2)
		n = n+1
	return sum
...
def structure_factor(h,k,l,wl,thB_S,alt, crystal_name):
    ...
    while i<len(el):
        ...
        FH[i] = f[i]*exp(-2j*pi*(h*xyz[i][:,0]+k*xyz[i][:,1]+l*xyz[i][:,2])).sum()
        FmH[i] = f[i]*exp(2j*pi*(h*xyz[i][:,0]+k*xyz[i][:,1]+l*xyz[i][:,2])).sum()
...
def f_FH(h,k,l,wl,thB_S,alt, crystal_name):
	FH, FmH, F0 = structure_factor(h,k,l,wl,thB_S,alt, crystal_name) #Replace name with appropriate material
	return FH, FmH, F0
