...
def randomize_old_FONCTIONNE(qv, T, D, fp_0, scale, limits): 
	...
	fp_t = fp_0[:]
	while (sum != len(fp_0)):
##		delta_fp = divide(tsallis_rv(qv, T, D), scale)
		for ii in range(len(fp_t)):
			sum2 = 0
			while (sum2 != 1):
...
#            pub.sendMessage(pubsub_Update_Gauge, val=0, emin=E_min, param=int(nb_minima))
    ...
    ## randomisation des paramètres
            fp_t, depassements = randomize(data['qv'], T, D, fp_0, par_scale, par_limits)
...
#                print(iteration, 'Emin = %4.3f' %E_min, '(',int(nb_minima), ')')
    ...
    ## sinon, si l'énergie a augementée, accepte le changemet que si random < Prob
                ...
                Pqa = accept_prob(Delta_E, T, data['qa'])
                alea = rand()
                if (alea < Pqa):
                    nb_rejet = 0
